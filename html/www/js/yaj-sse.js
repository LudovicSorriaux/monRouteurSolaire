...
var SSE = function (url, customSettings) {
    ...
    function yoCopy(source,dest){
        if (source.hasOwnProperty('onOpen')) dest.onOpen = source.onOpen;
        if (source.hasOwnProperty('onEnd')) dest.onEnd = source.onEnd;
        if (source.hasOwnProperty('onError')) dest.onError = source.onError;
        if (source.hasOwnProperty('onMessage')) dest.onMessage = source.onMessage;
        if (source.hasOwnProperty('options')) dest.options = source.options;
        if (source.hasOwnProperty('headers')) dest.headers = source.headers;
        if (source.hasOwnProperty('events')) dest.events = source.events;
    }
